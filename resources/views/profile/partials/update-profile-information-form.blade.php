    ...
    <form method="post" enctype="multipart/form-data" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        {{-- Update profile picture --}}
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl flex flex-col space-y-2 items-center">
                    <img src="{{ Storage::url( Auth::user()->image) }}" class='rounded-full overflow-hidden w-[150px] h-[150px] object-cover '>
                    <div class='mt-4'>
                        <label for="profile-picture" class="flex bg-gray-800 hover:bg-gray-700 text-white text-sm px-3 py-2 outline-none rounded w-max cursor-pointer mx-auto font-[sans-serif]">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-1 fill-white inline" viewBox="0 0 32 32">
                            ...
                            <input type="file" id='profile-picture' class="hidden" name='image' />
                        </label>
                    </div>
                </div>
            </div>
